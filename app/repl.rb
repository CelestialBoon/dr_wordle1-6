repl do 
  ...
  # puts "Loading general vectors..."
  # v_all ||= parse_vec(args.gtk.read_file("data/vec_all.txt"))
  
  # puts "Loading ratio vectors..."
  # v_ratio ||= parse_vec(args.gtk.read_file("data/vec_ratio.txt"))
  
  # puts "Loading possible answers..."
  # answers ||= args.gtk.read_file("data/answers.txt").split("\r\n")
  
  # puts "Loading other valid words..."
  # other_words ||= args.gtk.read_file("data/other_words.txt").split("\r\n")
  
  # puts "Loading valid responses..."
  # invalid_results ||= parse_inv(args.gtk.read_file("data/invalid_results.txt"))
  
  # puts "Loading tweets..."
  # tweets ||= parse_tweets(args.gtk.read_file("data/tweets.txt"))
...
#   # if day != 210 { next }
  ...
  list_length = v_all.length
  list_i = 0
  dists = {"all" => {}, "ratio" => {}, "invalid" => {}}
  v_all.each_key do |word|
    puts "Calculating distances... (#{list_i}/#{list_length})"
    list_i +=1
    dists["all"][word] = cosine(vec_all, v_all[word])
    dists["ratio"][word] = cosine(vec_ratio, v_ratio[word])
    dists["invalid"][word] = games.find_all {|g| !(g & invalid_results[word]).empty? }.length
  end
